...
GO
CREATE procedure [dbo].[ZWFOT12_Update_Proc] 
(
...
begin
	...
	if @p_STATUS = 'OT12_60'
		set @p_STATUS = 'OT12_61' --odblokowanie dokumentu (procedura zakłada nowe pozycje do integracji)
	...
	--rzutowanie zmiennych z SAP (POSKI jest polem jedynie do wyświetlania (zadanie inwestycyjne to wyjątek, HERST to text)
	...
	set @v_date = getdate()
			...
			if not exists (select 1 from dbo.KLASYFIKATOR5 (nolock) where KL5_CODE = @p_GDLGRP_POSKI) begin
				select @v_errorcode = 'OT12_001' -- Wprowadzono niewłaściwego użytkownika
				goto errorlabel end
		
			if not exists (select 1 from dbo.COSTCODE (nolock) where CCD_CODE = @p_KOSTL_POSKI) begin
				select @v_errorcode = 'OT12_002' -- Wprowadzono niewłaściwy numer MPK
				goto errorlabel end
			...
			begin
				select @v_errorcode = 'OT12_003' -- czy użytkownik zgadza sie z mpk
				goto errorlabel
			end
		
			if not exists (select 1 from STATION (nolock) where STN_KL5ID = @v_KL5ID and STN_ROWID = @p_MUZYTKID)
			begin
				select @v_errorcode = 'OT12_005' -- Wybrana stacja paliw (miejsce użytkowania) nie jest powiązana z klasyfikatorem 5 (użytkownikiem)
				goto errorlabel
			end
	...
	begin
		...
		BEGIN TRY
			...
			select 
				@P_STATUS, @v_RSTATUS, @p_ID, @p_ORG, LEFT(NEWID(),5), 'SAPO_ZWFOT12', GETDATE(), @p_UserID, @v_ITSID, @v_PSPID, @p_INOID
		...
		BEGIN TRY
			...
			insert into dbo.SAPO_ZWFOT12
			(  
				OT12_KROK, OT12_BUKRS, OT12_IMIE_NAZWISKO
				, OT12_SERNR, OT12_SERNR_POSKI
				, OT12_POSNR, OT12_POSNR_POSKI
				, OT12_HERST, OT12_HERST_POSKI, OT12_NR_DOW_DOST
				, OT12_KOSTL, OT12_KOSTL_POSKI
				, OT12_GDLGRP, OT12_GDLGRP_POSKI
				, OT12_ANLKL, OT12_ANLKL_POSKI
				, OT12_DATA_DOST
				, OT12_MUZYTKID, OT12_MUZYTK, OT12_PRZEW_OKRES, OT12_WOJEWODZTWO, OT12_IF_STATUS, OT12_IF_SENTDATE, OT12_IF_EQUNR, OT12_ZMT_ROWID, OT12_SAPUSER, OT12_MIES_DOST, OT12_ROK_DOST, ZMT_OBJ_CODE
				, OT12_PODZ_USL_P, OT12_PODZ_USL_S, OT12_PODZ_USL_B, OT12_PODZ_USL_C, OT12_PODZ_USL_U, OT12_PODZ_USL_H
				, OT12_CZY_FORM, OT12_ZZ_NR_FORM, OT12_ZZ_TYP_DOK, OT12_ZZ_POZ_FORM, OT12_ZZ_NR_DOK
			...
			select  
				...
				, @v_HERST, @p_HERST_POSKI, @p_NR_DOW_DOST
				, @v_KOSTL,	@p_KOSTL_POSKI
				, @v_GDLGRP, @p_GDLGRP_POSKI
				, @v_ANLKL,	@p_ANLKL_POSKI
				, @p_DATA_DOST
				, @p_MUZYTKID, @v_MUZYTK, @p_PRZEW_OKRES, @p_WOJEWODZTWO, 0,  NULL, NULL, @v_OTID, @p_SAPUSER, @p_MIES_DOST, @p_ROK_DOST, @p_ZMT_OBJ_CODE
				, @p_PODZ_USL_P, @p_PODZ_USL_S, @p_PODZ_USL_B, @p_PODZ_USL_C, @p_PODZ_USL_U, @p_PODZ_USL_H
				, @p_CZY_FORM, @p_ZZ_NR_FORM, @p_ZZ_TYP_DOK, @p_ZZ_POZ_FORM, @p_ZZ_NR_DOK
				
			select @v_OT12ID = SCOPE_IDENTITY()

		END TRY
		BEGIN CATCH
			select @v_syserrorcode = error_message()
			select @v_errorcode = 'OT12_002' -- blad wstawienia
			goto errorlabel
		END CATCH;

		--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		--pozycje z kompletu ZMT (uzupełnianie tylko podczas wprowadzania nowego OT12, aktualizacja poprzez inne formatki)
		--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		BEGIN TRY
			...
			order by STS_SETTYPE desc, STS_DESC
			
			-- aktualizacja lini składnika
			declare @v_OTLID int
			select @v_OTLID = IDENT_CURRENT('ZWFOTLN')
			update OBJ set OBJ_OTLID = @v_OTLID where OBJ_ROWID = @v_OBJID
			
			---------------------------------------------------------------------------------
			--------------------linie kompletacji OT do integracji w SAP---------------------
			---------------------------------------------------------------------------------
			insert into dbo.SAPO_ZWFOT12LN
			(  
				OT12LN_INVNR_NAZWA, OT12LN_CHAR_SKLAD, OT12LN_WART_ELEME, OT12LN_ANLN1, OT12LN_ANLN2, OT12LN_ZMT_ROWID, OT12LN_OT12ID, OT12LN_ZMT_OBJ_CODE
			) 		
			select

				left(OBJ_DESC,50), OBJ_NOTE, OBJ_VALUE, NULL, NULL, NULL, @v_OT12ID, OBJ_CODE
			from OBJ (nolock) 
				join STENCIL (nolock) on STS_ROWID = OBJ_STSID
			where OBJ_PARENTID = @v_OBJID
				and OBJ_PARENTID <> OBJ_ROWID --nie rozliczamy oddzielnie głównego składnika
				--and STS_SETTYPE = 'EKOM'
			order by STS_SETTYPE desc, left(OBJ_DESC,50)
			...
			---------------------------------------------------------------------------------------------------------------------
			--------------------uzupełnia OT12LN_ZMT_ROWID (takie trochę przesadzone, -------------------------------------------
			...
			---------------------------------------------------------------------------------------------------------------------
			update SAP 
				set SAP.OT12LN_ZMT_ROWID = ZMT.OTL_ROWID
			from dbo.ZWFOTLN ZMT  (nolock)
				join dbo.OBJ (nolock) on OBJ.OBJ_ROWID = ZMT.OTL_OBJID
				join dbo.SAPO_ZWFOT12LN SAP(nolock) on SAP.OT12LN_ZMT_OBJ_CODE = OBJ.OBJ_CODE
			where	
				SAP.OT12LN_OT12ID = @v_OT12ID
	...
	begin
		...
		--Sprawdzanie czy jest przynajmniej dwa składniki do kompletacji
		if 
			(select count(*) from ZWFOT12LNv join dbo.SAPO_ZWFOT12 (nolock) on OT12_ROWID = OT12LN_OT12ID where SAPO_ZWFOT12.OT12_ZMT_ROWID = @p_ZMT_ROWID ) < 2
			and @p_STATUS = 'OT12_20'
		begin
			select @v_syserrorcode = error_message()
			select @v_errorcode = 'OT12_004' -- blad wspoluzytkowania
			goto errorlabel
		end  
		...
		BEGIN TRY
		   ...
		   begin    
			...
			if @p_STATUS = 'OT12_20' 
			and exists (select 1 from dbo.ZWFOT12LNv (nolock) join dbo.SAPO_ZWFOT12 (nolock) on OT12_ROWID = OT12LN_OT12ID where isnull(OT12LN_WART_ELEME,0) = 0 and SAPO_ZWFOT12.OT12_ZMT_ROWID = @p_ZMT_ROWID )
			--sa zera w liniach kompletacji
	 		begin
				select @v_syserrorcode = error_message()
				select @v_errorcode = 'OT12_LN_001' -- blad wspoluzytkowania
				goto errorlabel
			end   
			
			--w takim przypadku wysyła nowy dokument (wraca ze statusu OT12_60 - Anulowany, z SAPa dostał status 9)
			if  @p_STATUS_old = 'OT12_60' and @p_STATUS_old <> @p_STATUS and @p_IF_STATUS in (3,9)
			begin
				...
				select 
					...
					, @v_ANLKL,	@p_ANLKL_POSKI
					, @p_DATA_DOST
					, @p_MUZYTKID, @v_MUZYTK, @p_PRZEW_OKRES, @p_WOJEWODZTWO, @v_IF_STATUS, NULL, NULL, @p_ZMT_ROWID, @p_SAPUSER, @p_MIES_DOST, @p_ROK_DOST, @p_ZMT_OBJ_CODE 
					, @p_PODZ_USL_P, @p_PODZ_USL_S, @p_PODZ_USL_B, @p_PODZ_USL_C, @p_PODZ_USL_U, @p_PODZ_USL_H
					, @p_CZY_FORM, @p_ZZ_NR_FORM, @p_ZZ_TYP_DOK, @p_ZZ_POZ_FORM, @p_ZZ_NR_DOK
								 
				select @v_OT12ID = IDENT_CURRENT('SAPO_ZWFOT12')
				...
				select
					OT12LN_INVNR_NAZWA, OT12LN_CHAR_SKLAD, OT12LN_WART_ELEME, OT12LN_ANLN1, OT12LN_ANLN2, OT12LN_ZMT_ROWID, @v_OT12ID, OT12LN_ZMT_OBJ_CODE
				...
				where 
					OT12_ZMT_ROWID = @p_ZMT_ROWID 
					and OT12_IF_STATUS in (3,9)
					and isnull(OTL_NOTUSED,0) = 0   
					
				--ustawienie statusu 4 historia
				update dbo.SAPO_ZWFOT12 SET
 					OT12_IF_STATUS = 4  --archiwum
				where 
					OT12_ZMT_ROWID = @p_ZMT_ROWID 
					and OT12_IF_STATUS in (3,9)
			...
			--nagłówek integracji (aktualizacja dla nagłówka integracji)
				...
				(@p_STATUS_old <> 'OT12_61' and @p_STATUS in ('OT12_10','OT12_20'))
				or (@p_STATUS_old = 'OT12_61' and @p_STATUS in ('OT12_20','OT12_61'))  
			begin
 
				UPDATE dbo.SAPO_ZWFOT12 SET
					OT12_KROK = @p_KROK, OT12_BUKRS = @p_BUKRS, OT12_IMIE_NAZWISKO = @p_IMIE_NAZWISKO
					, OT12_SERNR = @v_SERNR, OT12_SERNR_POSKI = @p_SERNR_POSKI
					, OT12_POSNR = @v_POSNR, OT12_POSNR_POSKI = @p_POSNR_POSKI
					, OT12_HERST = @v_HERST, OT12_HERST_POSKI = @p_HERST_POSKI, OT12_NR_DOW_DOST = @p_NR_DOW_DOST
					, OT12_KOSTL = @v_KOSTL, OT12_KOSTL_POSKI = @p_KOSTL_POSKI
					, OT12_GDLGRP = @v_GDLGRP, OT12_GDLGRP_POSKI = @p_GDLGRP_POSKI
					, OT12_ANLKL = @v_ANLKL, OT12_ANLKL_POSKI = @p_ANLKL_POSKI
					, OT12_DATA_DOST = @p_DATA_DOST
					, OT12_MUZYTKID = @p_MUZYTKID, OT12_MUZYTK = @v_MUZYTK, OT12_PRZEW_OKRES = @p_PRZEW_OKRES, OT12_WOJEWODZTWO = @p_WOJEWODZTWO, OT12_IF_STATUS = @v_IF_STATUS, OT12_IF_SENTDATE = @p_IF_SENTDATE, OT12_IF_EQUNR = @p_IF_EQUNR, OT12_SAPUSER = @p_SAPUSER, OT12_MIES_DOST = @p_MIES_DOST, OT12_ROK_DOST = @p_ROK_DOST, ZMT_OBJ_CODE = @p_ZMT_OBJ_CODE 
 					, OT12_PODZ_USL_P = @p_PODZ_USL_P, OT12_PODZ_USL_S = @p_PODZ_USL_S, OT12_PODZ_USL_B = @p_PODZ_USL_B, OT12_PODZ_USL_C = @p_PODZ_USL_C, OT12_PODZ_USL_U = @p_PODZ_USL_U, OT12_PODZ_USL_H = @p_PODZ_USL_H
					, OT12_CZY_FORM = @p_CZY_FORM, OT12_ZZ_NR_FORM = @p_ZZ_NR_FORM, OT12_ZZ_TYP_DOK = @p_ZZ_TYP_DOK, OT12_ZZ_POZ_FORM = @p_ZZ_POZ_FORM, OT12_ZZ_NR_DOK = @p_ZZ_NR_DOK
					 			
				where 
					OT12_ZMT_ROWID = @p_ZMT_ROWID 
					and OT12_IF_STATUS not in (3,4) --nie aktualizuje historycznych (IF_STATUS = 4)
