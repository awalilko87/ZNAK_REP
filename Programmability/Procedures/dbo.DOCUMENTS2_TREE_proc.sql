...
CREATE procedure [dbo].[DOCUMENTS2_TREE_proc]      
 @p_Entity nvarchar(4),      
 @p_PK1 nvarchar(30),      
 @p_PK2 nvarchar(30),      
...
BEGIN      
 ...
 FileID2+' <a href="' + dbo.VS_EncryptLink('GetFile.aspx?O=G&P=Files&ID='+FileID2) + '" target="_blank">'+isnull(FileName,'')+'</a>'+(case when isnull(FileName,'') <> isnull(Description,'') then isnull(' '+Description,'') else '' end) as DOT_DESC,      
 2 DOT_LEVEL,      
 FileID2,      
 DAE_CODE      
 from dbo.SYFiles (nolock)      
 inner join dbo.DOCENTITIES (nolock) on [FileID2] = DAE_DOCUMENT       
 where DAE_ENTITY = @v_Entity and isnull(DAE_MN,0) <> -1 and DAE_CODE = @v_Code      
...
BEGIN      
 ...
 isnull(DAE_CODE,'')  + ' ' + FileID2+' <a href="' + dbo.VS_EncryptLink('GetFile.aspx?O=G&P=Files&ID='+FileID2) + '" target="_blank">'+isnull(FileName,'')+'</a>'+(case when isnull(FileName,'') <> isnull(Description,'') then isnull(' '+Description,'')   
 else    
 '' end) as DOT_DESC,      
 2 DOT_LEVEL,      
 FileID2,      
...
BEGIN      
 ...
 select * from (      
  ...
  from dbo.SYFiles (nolock)      
  inner join dbo.DOCENTITIES (nolock) on [FileID2] = DAE_DOCUMENT       
  where DAE_ENTITY = 'VEN' and DAE_CODE in (select OFV_VENDOR+'#'+OFV_ORG from OFFVENDORv where OFV_OFFID in (select ROWID from dbo.OFFQRY where OFF_CODE = @p_PK1 and OFF_ORG = @p_PK2)) and isnull(DAE_MN,0) <> -1      
...
BEGIN      
 ...
 select      
 [FileID2] as DOT_CHILD,       
 N'Protokoły oceny technicznej' as DOT_PARENT,       
 FileID2+' <a href="' + dbo.VS_EncryptLink('GetFile.aspx?O=G&P=Files&ID='+FileID2) + '" target="_blank">'+isnull(FileName,'')+'</a>'+(case when isnull(FileName,'') <> isnull(Description,'') then isnull(' '+Description,'') else '' end) as DOT_DESC,      
 2 DOT_LEVEL,      
 FileID2,      
 DAE_CODE      
...
BEGIN     
 ...
 from dbo.VS_LangMsgs(nolock) where LangID = 'PL' and ControlID = '' and ObjectType = 'DOC_TREE' and ObjectID = N'Dokumenty OT'      
 union all    
  select 'Składnik' as DOT_CHILD, null as DOT_PARENT, '<input TYPE="image" SRC="/Images/16x16/Folder.png"> <b>'+Caption+'</b>' as DOT_DESC, 1 DOT_LEVEL, '' FileID2, '' DAE_CODE      
 from dbo.VS_LangMsgs(nolock) where LangID = 'PL' and ControlID = '' and ObjectType = 'DOC_TREE' and ObjectID = 'ASTOBJ'    
 ...
 from dbo.SYFiles (nolock)      
 inner join dbo.DOCENTITIES (nolock) on [FileID2] = DAE_DOCUMENT       
 where DAE_ENTITY = 'OT11' and isnull(DAE_MN,0) <> -1 and DAE_CODE in (select OBJ_CODE +'#'+ OBJ_ORG from OBJ    
...
BEGIN      
 ...
 from dbo.SYFiles (nolock)      
 inner join dbo.DOCENTITIES (nolock) on [FileID2] = DAE_DOCUMENT       
 where DAE_ENTITY = 'OT21' and isnull(DAE_MN,0) <> -1 and DAE_CODE in (select OBJ_CODE +'#'+ OBJ_ORG from OBJ    
 left join ZWFOTOBJ on OTO_OBJID = OBJ_ROWID  
 left join ZWFOT on OTO_OTID = OT_ROWID  
 where OT_CODE = @p_PK1)  
