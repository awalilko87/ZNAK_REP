...
GO
CREATE procedure [dbo].[OBJ_XLS_IMPORT_old]   
(  
  ...
  begin  
   select @SQL_COLUMNS =    
    'select top 1 @v_STS = ' + @v_STS_COL +  
     case when @v_PM_COL is not null then ', @v_PM = ' + @v_PM_COL else '' end +  
     case when @v_PM_PARENT_COL is not null then ', @v_PM_PARENT = ' + @v_PM_PARENT_COL else '' end +  
     case when @v_PM_NAME_COL is not null then ', @v_PM_NAME = ' + @v_PM_NAME_COL else '' end +  
     case when @v_PM_SERVICE_COL is not null then ', @v_PM_SERVICE = ' + @v_PM_SERVICE_COL else '' end +  
     case when @v_VALUE_COL is not null then ', @v_VALUE = ' + @v_VALUE_COL else '' end +  
     case when @v_SERIAL_COL is not null then ', @v_SERIAL = ' + @v_SERIAL_COL else '' end +  
     case when @v_DATE_COL is not null then ', @v_DATE = ' + @v_DATE_COL else '' end +  
     case when @v_AST_COL is not null then ', @v_AST = ' + @v_AST_COL else '' end +  
     case when @v_AST_SUBCODE_COL is not null then ', @v_AST_SUBCODE = ' + @v_AST_SUBCODE_COL else '' end +  
    ' from STN_ADD_OBJ_EXCEL (nolock) where ROWID = '  
    + cast(@c_ROWID as nvarchar(30))+ ''  
      
   exec sp_executesql   
    @query = @SQL_COLUMNS,  
    @params = N'@v_STS nvarchar(30) OUTPUT, @v_PM nvarchar(30) OUTPUT, @v_PM_PARENT nvarchar(30) OUTPUT, @v_PM_NAME nvarchar(30) OUTPUT, @v_PM_SERVICE nvarchar(30) OUTPUT, @v_VALUE nvarchar(30) OUTPUT, @v_SERIAL nvarchar(30) OUTPUT, @v_DATE nvarchar(30) O
UTPUT, @v_AST nvarchar(30) OUTPUT, @v_AST_SUBCODE nvarchar(30) OUTPUT',  
    @v_STS = @v_STS OUTPUT, @v_PM = @v_PM OUTPUT, @v_PM_PARENT = @v_PM_PARENT OUTPUT, @v_PM_NAME = @v_PM_NAME OUTPUT, @v_PM_SERVICE = @v_PM_SERVICE OUTPUT, @v_VALUE = @v_VALUE OUTPUT, @v_SERIAL = @v_SERIAL OUTPUT, @v_DATE = @v_DATE OUTPUT, @v_AST = @v_AST
 OUTPUT, @v_AST_SUBCODE = @v_AST_SUBCODE OUTPUT   
  ...
  select @v_STN_CCDID = STN_CCDID from STATION (nolock) where STN_ROWID = @p_STNID  
  select @v_AST_CCDID = AST_CCDID from ASSET (nolock) where AST_ROWID = @v_ASTID  
  select @v_AST_CCD = CCD_CODE from COSTCODE (nolock) where CCD_ROWID = @v_AST_CCDID  
  
  if   
   not exists (select 1 from dbo.OBJ where   
   OBJ_ROWID in (select OSA_OBJID from OBJSTATION where OSA_STNID = @p_STNID and OSA_KL5ID = @STN_KL5ID)  
  ...
  begin  
   ...
   select @v_STS_TYPE = STS_TYPE from STENCIL where STS_ROWID = @v_STSID  
   ...
   begin  
    select top 1 @v_PARENTID = OBJ_ROWID from OBJ (nolock) where OBJ_ROWID = OBJ_PARENTID and OBJ_STSID = 23  
    and OBJ_ROWID in (select OSA_OBJID from OBJSTATION where OSA_STNID = @p_STNID)       
   end  
  ...
  if isnull(@v_AST_CCDID, @v_STN_CCDID) <> @v_STN_CCDID  
   ...
   else if exists (select 1 from dbo.OBJ   
  ...
  else if @p_STNID is null   
   print 'Nie podano numeru stacji paliw'  
  else if @v_STSID is null  
   print 'Nie podano kodu szablonu'  
    
    
  --------------------------------------------------------------------------------------------------------------------------------------------------------  
  select @v_OBJID = OBJ_ROWID from OBJ (nolock) where OBJ_PM = @v_PM  
  --------------------------------------------------------------------------------------------------------------------------------------------------------  
  ---------------------------------------------------------------PARAMETRY--------------------------------------------------------------------------------  
  --------------------------------------------------------------------------------------------------------------------------------------------------------    
  ...
  begin  
   ...
   declare c_OBJ_PV cursor static for -- kursor 2 c_OBJ_PV  
