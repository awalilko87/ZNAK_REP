...
GO
CREATE procedure [dbo].[OBJ_XLS_IMPORT_MULTI]             
(                  
...
-- select @p_UserID = 'SA'        
 ...
 declare              
  ...
  @v_STN_COL nvarchar(50),      
  @v_STN nvarchar(30),       
  @v_STNID int,        
  @v_SUBMPK_COL nvarchar(30),      
  @v_SUBMPK nvarchar(30),            
  @v_AST_CCDID int,             
  @v_AST_CCD nvarchar(30),             
  @v_STN_CCDID int,             
  @v_STS_TYPE nvarchar(30),       
  @ci varbinary(128),
  @v_STN_BRAK nvarchar(max),       
  ...
  set @ci = convert(varbinary(128),'NOPM')
  set context_info @ci
 ...
 delete from STN_ADD_OBJ_EXCEL where c03 in ('Nazwa SAP>' ,'Kod SAP>') -- usunięcie z pliku Excel dwóch pierwszych rekordów z kodami            
 ...
 select top 1 @v_ROWID_NAG = ROWID from STN_ADD_OBJ_EXCEL (nolock) where ImportUser = @p_UserID order by rowid asc
  ...
  ;with stnb as (select distinct STN = c01 from dbo.STN_ADD_OBJ_EXCEL where ImportUser = @p_UserID and ROWID <> @v_ROWID_NAG and not exists (select 1 from dbo.STATION where STN_CODE = c01))
  select top 10000 @v_STN_BRAK = isnull(@v_STN_BRAK,'') + ',' + STN from stnb order by STN

  if len(@v_STN_BRAK) > 1
  begin
	raiserror('Brak stacji %s w systemie. Dodaj stację i ponów import składników.', 16, 1, @v_STN_BRAK)
	return 1
  end
 ...
 begin            
  ...
  /*Przypisujemy kolumny z XLS do poszczególnych zmiennych*/          
    /*Stąd pobierzemu*/      
  exec dbo.[GetTableColumnName] @v_ROWID_NAG, N'kod stacji', N'STN_ADD_OBJ_EXCEL', @p_COLUMNNAME = @v_STN_COL OUTPUT         
  exec dbo.[GetTableColumnName] @v_ROWID_NAG, N'Submpk', N'STN_ADD_OBJ_EXCEL', @p_COLUMNNAME = @v_SUBMPK_COL OUTPUT             
 ...
 --  print     @v_PM_COL      
  ...
  begin            
    ...
    /*Kod stacji*/  case when @v_STN_COL is not null then ', @v_STN = ' + @v_STN_COL else '' end +      
    /*Submpk*/  case when @v_SUBMPK_COL is not null then ', @v_SUBMPK = ' + @v_SUBMPK_COL else '' end +      
  ...
  /*Wartość nabycia*/  case when @v_VALUE_COL is not null then ', @v_VALUE = ' + @v_VALUE_COL else '' end +            
   ...
   exec sp_executesql             
    @query = @SQL_COLUMNS,            
    @params = N'@v_STS nvarchar(30) OUTPUT,@v_STN nvarchar(30) OUTPUT, @v_SUBMPK nvarchar(30) OUTPUT, @v_PM nvarchar(30) OUTPUT          
 ...
 --print @v_PM      
            ...
            --select @p_STNID = STN_ID from STATION where STN_CODE = @v_STN      
            
 /*Wyciągamy rowid składnika na podtawie jego kodu */        
       
              
    set @SQL_STNID =  'select top 1 @v_STNID = STN_ROWID from STATION (nolock) join KLASYFIKATOR5 on KL5_ROWID = STN_KL5ID where KL5_CODE = ''' + @v_SUBMPK + ''''            
    exec sp_executesql             
    @query = @SQL_STNID,            
    @params = N'@v_STNID int OUTPUT',            
    @v_STNID = @v_STNID OUTPUT        
   ...
   --set @Mes = 'Składnik:' + cast(@v_STSID as nvarchar(10))      
   -- print @Mes      
  ...
  /*SPRAWDZAMY, CZY DLA DANYCH PARAMETRÓW ISTNIEJE ODNIESIENIE W TABELI PROPERTY VALUES*/          
   ...
   EXEC CHECK_PROPERTY_FOR_STS @v_STSID, @v_PM, @p_UserID,@v_STNID;          
            
  /*----------------------------------------------------------------------------------*/          
               
  select @v_STN_CCDID = STN_CCDID from STATION (nolock) where STN_ROWID = @v_STNID  --id  MPK przypisanego do stacji          
...
/*jeżeli nie istnieje żaden obiekt z tej stacji i tej klasyfikacji 5*/           
  ...
  if             
   ...
   OBJ_ROWID in (select OSA_OBJID from OBJSTATION  join KLASYFIKATOR5 on KL5_ROWID = OSA_KL5ID          
  where OSA_STNID = @v_STNID and KL5_CODE = @v_SUBMPK)            
          
   and OBJ_PM = isnull(@v_PM,'!@#$_%^_&*(') and OBJ_PM <> '' )             
   and @v_STSID is not null             
   and @v_STNID is not null            
  ...
  begin            
   ...
   if not exists (select * from OBJ where OBJ_ROWID = OBJ_PARENTID and OBJ_STSID = 23            
    and OBJ_ROWID in (select OSA_OBJID from OBJSTATION where OSA_STNID = @v_STNID))            
  ...
  if coalesce(@v_AST_CCDID, @v_STN_CCDID,-89) <> isnull(@v_STN_CCDID, -88)
 
  print 'Niezgodny MPK środka trwałego z MPK stacji paliw: ' + isnull(@v_AST, 'BRAK') + '(MPK: ' + isnull(@v_AST_CCD, 'BRAK_') +') stacja paliw: ' +  @v_STN + ' Składnik: '  +  @v_PM + ' - ' + @v_PM_NAME        
            
            
   else           
             
   if exists (select 1 from dbo.OBJ             
   where OBJ_ROWID in (select OSA_OBJID from OBJSTATION join KLASYFIKATOR5 on KL5_ROWID = OSA_KL5ID          
										where OSA_STNID = @v_STNID and KL5_CODE = @v_SUBMPK)            
   and OBJ_PM = isnull(@v_PM,'!@#$_%^_&*(') and OBJ_PM <> '' )         
  ...
  if @v_STSID is null           
  print 'Nie podano kodu szablonu'   + @v_STS         
