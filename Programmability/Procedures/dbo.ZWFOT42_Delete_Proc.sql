...
GO
CREATE procedure [dbo].[ZWFOT42_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			set POL_OT42ID = NULL
			where POL_OT42ID = @v_OTID
		
			delete from dbo.SAPO_ZWFOT42LN where OT42LN_OT42ID in (select OT42_ROWID from dbo.SAPO_ZWFOT42 where OT42_ZMT_ROWID = @v_OTID)
			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID 
			delete from dbo.ZWFOTLN where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT42 where OT42_ZMT_ROWID = @v_OTID
