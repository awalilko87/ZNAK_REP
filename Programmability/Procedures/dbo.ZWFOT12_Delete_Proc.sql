...
GO
CREATE procedure [dbo].[ZWFOT12_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			delete from dbo.SAPO_ZWFOT12LN where OT12LN_OT12ID in (select OT12_ROWID from dbo.SAPO_ZWFOT12 where OT12_ZMT_ROWID = @v_OTID)
			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID
			delete from dbo.ZWFOTLN  where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT12 where OT12_ZMT_ROWID = @v_OTID
