...
GO
CREATE procedure [dbo].[INVTSK_ADD_OBJ_XLS_Verify2]   
(   
@p_UserID nvarchar(30)  
 ...
 select rowid, c01, c02, c03, replace(replace(nullif(c04,''),',','.'),' ',''), replace(replace(nullif(c05,''),',','.'),' ','') from dbo.INVTSK_ADD_OBJ_XLS where importuser = @p_UserID order by rowid  
 open c_XLS  
 fetch next from c_XLS into @c_ROWID, @c_STN, @c_STS, @c_PARENT_OBJ, @c_QTY, @c_VALUE  
 ...
 begin  
  ...
  if not exists (select 1 from dbo.STENCIL where STS_CODE = @c_STS and STS_NOTUSED = 0)  
  begin  
