...
GO
CREATE procedure [dbo].[ZWFOT31_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			set POL_OT31ID = NULL
			where POL_OT31ID = @v_OTID
		
			delete from dbo.SAPO_ZWFOT31LN where OT31LN_OT31ID in (select OT31_ROWID from dbo.SAPO_ZWFOT31 where OT31_ZMT_ROWID = @v_OTID)
			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID
			delete from dbo.ZWFOTLN  where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT31 where OT31_ZMT_ROWID = @v_OTID
