...
GO
CREATE procedure [dbo].[ZWFOT40_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			delete from dbo.SAPO_ZWFOT40LN where OT40LN_OT40ID in (select OT40_ROWID from dbo.SAPO_ZWFOT40 where OT40_ZMT_ROWID = @v_OTID)
			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID
			delete from dbo.ZWFOTLN  where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT40 where OT40_ZMT_ROWID = @v_OTID
