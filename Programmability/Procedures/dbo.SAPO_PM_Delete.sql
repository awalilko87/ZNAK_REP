...
GO
CREATE procedure [dbo].[SAPO_PM_Delete] (
@p_OBJID int, 
...
begin
	...
	begin
		...
		declare @datateraz datetime = getdate()
		INSERT INTO dbo.TestTrigger (obj, statuss, [userid], [operacja], data_inse)
		select @p_OBJID, ' insert w procedurze sapo_pm_delete parametr' , (select 1 from OBJ(nolock) where OBJ_ROWID = @p_OBJID and OBJ_PM_TOSEND = 1 and isnull(OBJ_PM, '') = case @p_OPER_TYPE when 'U' then OBJ_PM when 'I' then '' end), 'D', @datateraz;



	if exists (select 1 from OBJ(nolock) where OBJ_ROWID = @p_OBJID and OBJ_PM_TOSEND = 1 and isnull(OBJ_PM, '') = case @p_OPER_TYPE when 'U' then OBJ_PM when 'I' then '' end /*and OBJ_PSPID is not null*/)
	begin
		...
		INSERT INTO dbo.TestTrigger (obj, statuss, [userid], [operacja], data_inse)
		--VALUES(OBJ_ROWID, 'insert w procedurze sapo_pm_delete', @UserID,'D', @datateraz )
		select @p_OBJID, 'insert w procedurze sapo_pm_delete parametr' , @p_UserID, 'D', @datateraz;
		--from OBJ (nolock)
		--where 
		--	/*OBJ_PSPID is not null
		--	and */OBJ_ROWID = @p_OBJID
		--select * from sapo_pm order by pm_rowid desc
		INSERT INTO dbo.TestTrigger (obj, statuss, [userid], [operacja], data_inse)
		--VALUES(OBJ_ROWID, 'insert w procedurze sapo_pm_delete', @UserID,'D', @datateraz )
		select OBJ_ROWID, 'insert w procedurze sapo_pm_delete obj_rowid' , @p_UserID, 'D', @datateraz
		from OBJ (nolock)
		where 
			/*OBJ_PSPID is not null
			and */OBJ_ROWID = @p_OBJID;
		...
		unpivot
			(VAL for COL in
				([EQUI-STTXT], [EQUI-SERGE], [EQUI-EQUNR], [IBIPEQUI-EQKTX], [EQUI-ANLNR], [EQUI-ANLUN])
		) ou
