...
GO
CREATE procedure [dbo].[POL_Update_Proc_Zes]        
(        
@p_POTID int,        
@p_TO_STNID int,        
...
begin         
 ...
 /*Dla składników typu zestaw ustawiamy wspólny statsu dla wszystkich elementów*/    
...
if @STS_SETTYPE = 'ZES'     
 begin    
  ...
  declare @stn_to int  
  select @stn_to = case when @p_STATUS = 'POL_005' then @v_TO_STNID   
      when @p_STATUS = 'POL_003' then @v_TO_STNID  
      else null end  
  ...
  and POL_STATUS <> 'POL_007'  
  and POL_OBJID in     
   (select OBJ_ROWID from OBJ where OBJ_STSID in    
    (select OBJ_STSID from OBJ where OBJ_ROWID = @p_OBJID    
    union all       
    (select STL_CHILDID from STENCILLN where STL_PARENTID =(select OBJ_STSID from OBJ where OBJ_ROWID = @p_OBJID))  
    )  
  )    
