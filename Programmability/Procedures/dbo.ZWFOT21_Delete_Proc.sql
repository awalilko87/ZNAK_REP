...
GO
CREATE procedure [dbo].[ZWFOT21_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			delete from dbo.SAPO_ZWFOT21LN where OT21LN_OT21ID in (select OT21_ROWID from dbo.SAPO_ZWFOT21 where OT21_ZMT_ROWID = @v_OTID)
			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID
			delete from dbo.ZWFOTLN  where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT21 where OT21_ZMT_ROWID = @v_OTID
