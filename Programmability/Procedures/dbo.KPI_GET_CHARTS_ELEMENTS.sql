...
--exec KPI_GET_CHARTS_ELEMENTS 'SA'
create procedure [dbo].[KPI_GET_CHARTS_ELEMENTS](  
...
as  
  ...
  SELECT @v_org = SiteID, @v_GroupID = isnull(UserGroupID,'') from dbo.SYUsers (nolock) where UserID = @_UserID    
  
  DECLARE cKPI CURSOR STATIC FOR            
  SELECT KPI_ID, isnull(Caption,KPI_DESC), KPI_SOURCE, KPI_WHERE, KPI_DEFIMAGE, KPI_LINK, KPI_RPT, CHART_TYPE, KPI_FORMID, KPI_DATASPY, KPU_SHOW_WHEN_ZERO           
  ...
  BEGIN        
	IF isnull(@cr_link,'') <> '' BEGIN
		SELECT @sp_link = 'javascript:Simple_Popup(''/Forms/SimplePopup/' + isnull(@cr_link,'') + ''',null,''scrollbars=no,menubar=no,resizable=yes,fullscreen=no'');void(0);'          
	END
	ELSE BEGIN
		SELECT @sp_link = '#'
	END
