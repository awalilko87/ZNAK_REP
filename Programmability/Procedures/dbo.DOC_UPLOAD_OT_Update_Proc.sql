...
GO
CREATE procedure [dbo].[DOC_UPLOAD_OT_Update_Proc]       
...
-- select @p_FileName = [FileName] from dbo.SYFiles (nolock) where FileID2 = @p_FileID2      
 ...
 set @p_Description = ''      
   ...
   select  @kod = case when @p_TXT01 is not null then @p_TXT01 else @p_PK1 end  
      
  declare @v_code nvarchar(50)      
  select @v_code = dbo.GetFilesCodeFromEntity (@p_Entity,@kod,@p_PK2,@p_PK3)      
 ...
 begin      
  ...
  begin      
   insert into dbo.DOCENTITIES (DAE_DOCUMENT, DAE_ENTITY, DAE_TYPE, DAE_TYPE2, DAE_TYPE3, DAE_CODE, DAE_MN, DAE_CREUSER)      
    ...
    exec DOC_UPLOAD_AUDIT @v_FileCode, @v_FileName, 'NOWY',@kod, @p_UserID    
