...
GO
CREATE procedure [dbo].[ZWFOT33_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			set POL_OT33ID = NULL
			where POL_OT33ID = @v_OTID
		
			delete from dbo.SAPO_ZWFOT33DON where OT33DON_OT33ID in (select OT33_ROWID from dbo.SAPO_ZWFOT33 where OT33_ZMT_ROWID = @v_OTID)
			delete from dbo.SAPO_ZWFOT33LN where OT33LN_OT33ID in (select OT33_ROWID from dbo.SAPO_ZWFOT33 where OT33_ZMT_ROWID = @v_OTID)

			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID
			delete from dbo.ZWFOTDON where OTD_OTID = @v_OTID
			delete from dbo.ZWFOTLN  where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT33 where OT33_ZMT_ROWID = @v_OTID
