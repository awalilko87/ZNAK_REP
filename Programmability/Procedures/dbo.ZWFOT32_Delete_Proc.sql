...
GO
CREATE procedure [dbo].[ZWFOT32_Delete_Proc]
(
...
begin
	...
	begin
		...
		BEGIN TRY
			...
			set POL_OT32ID = NULL
			where POL_OT32ID = @v_OTID
		
			delete from dbo.SAPO_ZWFOT32LN where OT32LN_OT32ID in (select OT32_ROWID from dbo.SAPO_ZWFOT32 where OT32_ZMT_ROWID = @v_OTID)
			delete from dbo.ZWFOTOBJ where OTO_OTID = @v_OTID
			delete from dbo.ZWFOTLN  where OTL_OTID = @v_OTID 
			delete from dbo.SAPO_ZWFOT32 where OT32_ZMT_ROWID = @v_OTID
