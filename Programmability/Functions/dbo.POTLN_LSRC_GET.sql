...
GO
create   function [dbo].[POTLN_LSRC_GET](@p_POTID int)
returns table
as
return
SELECT
 POL_ROWID
,POL_LP = (row_number() over(partition by POL_POTID order by O.OBJ_CODE,PARENT.OBJ_CODE))
,POL_POTID
...
,OT31ID = case when OT_TYPE = 'SAPO_ZWFOT31' then OT_CODE else null end
,OT32ID = case when OT_TYPE = 'SAPO_ZWFOT32' then OT_CODE else null end
,OT33ID = case when OT_TYPE = 'SAPO_ZWFOT33' then OT_CODE else null end
,OT41ID = case when OT_TYPE = 'SAPO_ZWFOT41' then OT_CODE else null end
,OT42ID = case when OT_TYPE = 'SAPO_ZWFOT42' then OT_CODE else null end
,STS_DEFAULT_KST
,STS_SETTYPE
...
left join dbo.ZWFOT on OT_ROWID = coalesce(POL_OT31ID, POL_OT32ID, POL_OT33ID, POL_OT41ID, POL_OT42ID)
