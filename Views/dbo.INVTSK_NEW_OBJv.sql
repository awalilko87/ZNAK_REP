...
CREATE view [dbo].[INVTSK_NEW_OBJv]
as
select
  INO_ROWID
 , INO_STSID
 ...
 , OBJ_NOTE
 , OBJ_DATE
 , OBJ_TIME = null --(dbo.e2_im_gettimefromdatetime(isnull(OBJ_DATE,'1900-01-01')))
 , OBJ_STATUS
 , OBJ_STATUS_DESC = (select STA_DESC from STA (nolock) where STA_CODE = OBJ_STATUS and STA_ENTITY = 'OBJ')
 , OBJ_ICONSTATUS =  null --dbo.[GetStatusImage] ('OBJ', OBJ_STATUS)
 ...
 , OBJ_MANUFACID
 , OBJ_MANUFAC = null --(select MFC_CODE from dbo.MANUFAC (nolock) where MFC_ROWID = OBJ_MANUFACID)
 , OBJ_VENDORID
 , OBJ_ASSETID = AST_ROWID
 , OBJ_ASSET = AST_CODE
 , OBJ_ASSET_DESC = AST_DESC
 , OBJ_PERSON
 , OBJ_PERSON_DESC = null --dbo.EmpDesc(OBJ_PERSON,OBJ_ORG)
 , OBJ_PARTSLISTID
 , OBJ_PARTSLIST = null --(select OPL_CODE from dbo.OBJPARTSLIST (nolock) where OPL_ROWID = OBJ_PARTSLISTID)
 , OBJ_LOCID
 , OBJ_LOC  = null --(select LOC_CODE from dbo.LOCATION (nolock) where LOCATION.LOC_ROWID = OBJ_LOCID)
 , OBJ_CATALOGNO
 , OBJ_YEAR
 , OBJ_SERIAL
 ...
 , OBJ_COSTCENTERID
 , OBJ_MRCID
 , OBJ_MRC = null --(select MRC_CODE from dbo.MRC (nolock) where MRC_ROWID = OBJ_MRCID)
 ...
 , [OT_NEW_11] = 
	 case when (OT_ID is null or OT_TYPE = 'SAPO_ZWFOT11') and OSA_STNID is not null and STS_SETTYPE in ('ZES', 'EZES','SKL', 'EKOM') then --element kompletu można rozliczyć OT 11 (http://jira.eurotronic.net.pl/browse/PKNTA-161)
 ...
 , [OT_NEW_12] = 
	 case when (OT_ID is null or OT_TYPE = 'SAPO_ZWFOT12') and OSA_STNID is not null and ISNULL(INO_TXT12,'') <> 'DON' and STS_SETTYPE in ('KOM','EKOM') then
    '<a href="'+ dbo.VS_EncryptLink('/Tabs3.aspx/' +
    '?WHO=INVTSK_NEW_OBJ_LS'+
    '&FID=OT12_RC'+
    '&FLD=Btn_AddOT12'+
 ...
 , [OT_NEW_21] = 
	 case when (OT_ID is null or OT_TYPE = 'SAPO_ZWFOT21') and OSA_STNID is not null and ISNULL(INO_TXT12,'') <> 'DON' and STS_SETTYPE in ('ZES', 'KOM', 'EZES','SKL', 'EKOM') and OBJ_TXT12 is null then
    '<a href="'+ dbo.VS_EncryptLink('/Tabs3.aspx/' +
    '?WHO=INVTSK_NEW_OBJ_LS'+
    '&FID=OT21_RC'+
    '&FLD=Btn_AddOT21'+
 ...
 , OBJ_PARENT_CODE = (select OBJ_CODE from OBJ x where x.OBJ_ROWID = o.OBJ_PARENTID)
 , OBJ_STS_SETTYPE = (select STT_DESC from SETTYPE where STT_CODE = STENCIL.STS_SETTYPE)

from [INVTSK_NEW_OBJ] (nolock)
 join dbo.STENCIL (nolock) on STS_ROWID = INO_STSID
 join dbo.INVTSK (nolock) on ITS_ROWID = INO_ITSID
 left join dbo.OBJ o (nolock) on INO_ROWID = OBJ_INOID
 ...
 left join dbo.ZWFOTv (nolock) on /*OT_PSPID = PSP_ROWID and OT_ITSID = ITS_ROWID and*/ ((OBJ_OTID = OT_ROWID  /* and OT_STATUS<>'OT11_32'*/ ) or OT_INOID = INO_ROWID) -- and OT_STATUS not in ('OT11_70','OT12_70','OT21_70') --PKNTA-1963
 --left join dbo.ZWFOTv (nolock) on /*OT_PSPID = PSP_ROWID and OT_ITSID = ITS_ROWID and*/ case when OT_INOID is not null and OT_INOID = INO_ROWID then 1 
	--																						 when OT_INOID is null and OBJ_OTID = OT_ROWID and OT_INOID <> INO_ROWID THEN 1 ELSE 0 END=1
 outer apply (select top 1 AST_ROWID, AST_CODE, AST_DESC from dbo.ASSET inner join dbo.OBJASSET on OBA_ASTID = AST_ROWID where OBA_OBJID = OBJ_ROWID order by AST_SUBCODE)ast
where
