...
GO
-- select * from PO_REPORTv
CREATE view [dbo].[PO_REPORT_POZOST_LINESv]  
as
   SELECT
POL_POTID
,POL_LP
...
,  POL_STATUS_DESC = case POL_STATUS when 'POL_002' then 'Składniki przeznaczone do likwidacji poprzez utylizację'
									 ...
									 when 'POL_004' then 'Składniki pozostające na stacji'
...
,POL_NOTE  
,POL_GROUP = GroupID  
--,POL_VALUE = (select cast(AST_INITIALVALUE as numeric(30,6)) from dbo.ASSET (nolock) where AST_ROWID = oba.OBA_ASTID) --o.OBJ_VALUE  /*Wartość początkowa SAP*/
,POL_VALUE = (select cast( (case when AST_CODE like '3%' then AST_SAP_URWRT else  AST_INITIALVALUE end ) as numeric(30,6)) from dbo.ASSET (nolock) where AST_ROWID = oba.OBA_ASTID) --o.OBJ_VALUE  /*Wartość początkowa SAP*/
...
--,POL_BIZ_NETTO = case when POL_BIZ_DEC = 1 then (case when POL_PARTIAL=0 then 100 else  IsNull(POL_PARTIAL,100)   end)  * (select AST_NETVALUE from dbo.ASSET(nolock) where AST_ROWID = oba.OBA_ASTID)/100 else 0 end
--,POL_TECH_NETTO = case when POL_BIZ_DEC = 0 then (case when POL_PARTIAL=0 then 100 else  IsNull(POL_PARTIAL,100)   end)  * (select AST_NETVALUE from dbo.ASSET(nolock) where AST_ROWID = oba.OBA_ASTID)/100 else 0 end
