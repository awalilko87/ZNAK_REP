...
GO
CREATE view [dbo].[ASTOBJv]          
as          
select          
 --std          
  o.OBJ_ROWID          
, o.OBJ_PARENTID          
, OBJ_LP = 0 --(row_number() over(partition by '' order by p.OBJ_CODE, o.OBJ_CODE))          
...
, SP_OBJ_RC_LINK = '<a href="javascript:Simple_Popup('''           
  + dbo.VS_EncryptLink('/Forms/SimplePopup.aspx/?WHO=SP_OBJ_LS&FID=SP_OBJ_RC&FLD=SP_OBJ_RC_LINK&A=BTN_REFRESH&OBJ_ID=' + cast(o.OBJ_ID as nvarchar(50))) + ''',''mywindowtitle'')"          
...
--left join  dbo.OBJTECHPROT on POT_ROWID = POL_POTID            
 ...
 left join dbo.PSP (nolock) on PSP_ROWID = O.OBJ_PSPID    -- BYŁO WCZEŚNIEJ P.OBJ_PSPID       
