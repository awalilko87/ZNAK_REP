...
CREATE view [dbo].[PO_REPORT_LINES_ODSPRZv]   
...
,POL_STATUS_DESC = case POL_STATUS when 'POL_002' then N'Składniki przeznaczone do likwidacji poprzez utylizację' 


          when 'POL_003' then N'Składniki przeznaczone do zagospodarowania' 


          when 'POL_004' then N'Składniki przeznaczone do pozostawienia na stacji' 


          when 'POL_005' then N'Składniki przeznaczone do przeniesienia na inną stację' 


          when 'POL_006' then N'Składniki przeznaczone do tymczasowego demontażu' 


          when 'POL_007' then N'Składniki zablokowane' 


          when 'POL_008' then N'Składniki przeznaczone likwidacji poprzez odsprzedaż' 
...
,POL_VALUE = (select cast(AST_INITIALVALUE as numeric(30,6)) from dbo.ASSET where AST_ROWID = oba.OBA_ASTID) --o.OBJ_VALUE  
...
where pol_status in ('POL_008', 'CPOLN_008')
